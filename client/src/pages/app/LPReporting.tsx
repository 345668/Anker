...
interface FundAnalytics {
  portfolio?: { totalValue?: number; portfolioCount?: number };
  performance?: { tvpi?: number; dpi?: number };
}
...
function downloadReport(report: LpReport, analytics: FundAnalytics | null, fundName: string) {
  const lines = [
    `LP REPORT — ${report.title.toUpperCase()}`,
    "=".repeat(60),
    `Fund: ${fundName}`,
    `Type: ${REPORT_TYPE_LABELS[report.reportType] || report.reportType}`,
    `Status: ${report.status}`,
    `Created: ${new Date(report.createdAt).toLocaleDateString()}`,
    report.periodStart ? `Period Start: ${new Date(report.periodStart).toLocaleDateString()}` : "",
    report.periodEnd ? `Period End: ${new Date(report.periodEnd).toLocaleDateString()}` : "",
    "",
    "FUND METRICS",
    "-".repeat(40),
    `NAV: ${formatCurrency(analytics?.portfolio?.totalValue || 0)}`,
    `TVPI: ${(analytics?.performance?.tvpi || 0).toFixed(2)}x`,
    `DPI: ${(analytics?.performance?.dpi || 0).toFixed(2)}x`,
    `Portfolio Companies: ${analytics?.portfolio?.portfolioCount || 0}`,
    "",
    report.notes ? `NOTES\n${"-".repeat(40)}\n${report.notes}` : "",
    "",
    "=".repeat(60),
    `Generated by Anker Platform on ${new Date().toLocaleDateString()}`,
...
export default function LPReporting() {
  ...
  const { data: reports, isLoading: loadingReports } = useQuery<LpReport[]>({
    queryKey: ["/api/institutional/funds", selectedFundId, "reports"],
    enabled: !!selectedFundId,
  });

  const { data: fundAnalytics } = useQuery<FundAnalytics>({
    queryKey: ["/api/institutional/funds", selectedFundId, "analytics"],
    enabled: !!selectedFundId,
  });

  const selectedFund = (funds as Fund[] | undefined)?.find(f => f.id === selectedFundId);
  ...
  function openCreateWithType(type: string) {
    ...
    form.setValue("title", `${label} ${year}`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <Card className="bg-[rgb(30,30,30)] border-white/10 lg:col-span-1">
            ...
            <CardContent className="space-y-2">
              {(funds as Fund[] | undefined)?.map((fund) => (
                <div
                  key={fund.id}
              ...
              <>
                ...
                <Card className="bg-[rgb(30,30,30)] border-white/10">
                  <CardContent className="p-0">
                    ...
                    ) : (reports as LpReport[] | undefined)?.length ? (
                      <div className="divide-y divide-white/10">
                        {(reports as LpReport[]).map((report) => (
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  ...
                                  size="icon"
                                  className="text-white/60 hover:text-white"
                                  onClick={() => downloadReport(report, analytics as FundAnalytics, selectedFund?.name || "Fund")}
                ...
                <Card className="bg-[rgb(30,30,30)] border-white/10">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      <button
                        onClick={() => openCreateWithType("quarterly")}
                        className="p-4 rounded-lg bg-white/5 border border-white/10 hover:border-purple-500/50 hover:bg-white/10 transition-all text-left group"
                        data-testid="template-quarterly"
                      >
                      ...
                      <button
                        onClick={() => openCreateWithType("capital_call")}
                        className="p-4 rounded-lg bg-white/5 border border-white/10 hover:border-green-500/50 hover:bg-white/10 transition-all text-left group"
                        data-testid="template-capital-call"
                      >
                      ...
                      <button
                        onClick={() => openCreateWithType("distribution")}
                        className="p-4 rounded-lg bg-white/5 border border-white/10 hover:border-blue-500/50 hover:bg-white/10 transition-all text-left group"
                        data-testid="template-distribution"
                      >
                      ...
                      <button
                        onClick={() => openCreateWithType("annual")}
                        className="p-4 rounded-lg bg-white/5 border border-white/10 hover:border-orange-500/50 hover:bg-white/10 transition-all text-left group"
                        data-testid="template-annual"
                      >
