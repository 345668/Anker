...
export async function requireWorkspace(write = false) {
  ...
  const canWrite = ["workspace_owner", "admin", "member"].includes(active.orgRole) && active.persona !== "lp"
  if (write && !canWrite) throw new WorkspaceError("This workspace is read-only.")
  return { userId: user.id, orgId: active.orgId, name: active.name, persona: active.persona, canWrite, canSendOutreach: active.canSendOutreach, role: active.orgRole, kind: active.kind }
}
...
export function workspaceError(error: unknown) {
  ...
  if (["23514","23503","22P02","22007"].includes(code || "")) return NextResponse.json({error:"A field or linked record is invalid for this workspace. Review it and retry."},{status:400})
  if (code === "23505") return NextResponse.json({error:"This record already exists or changed in another request. Reload before retrying."},{status:409})
  console.error("[workspace operation]", {code})
