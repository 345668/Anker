...
export async function isAiAvailable(): Promise<boolean> {
  return isAvailable().catch(() => false)
}
...
export async function enrichFirmsWithRationales(
  ...
  if (!(await isAvailable().catch(() => false)) || targets.length === 0) {
    for (const f of firms) f.whyThisLp = ruleBasedRationaleFirm(f)
    return { enriched: 0 }
  }
  const prompts = targets.map((f) => buildFirmRationalePrompt(f, fund))
  const results = await generateBatch(prompts, { maxTokens: 80, temperature: 0.4, task: "ai_rationale" }, 4, onProgress).catch(() => prompts.map(() => ""))
  let enriched = 0
...
export function ruleBasedRationaleFirm(f: ScoredFirmV2): string {
  const bits: string[] = []
...
export function ruleBasedRationaleContact(ct: ScoredContactV2): string {
  const bits: string[] = []
