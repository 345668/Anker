import { requireCrmWorkspace as requireWorkspace } from "@/lib/crm/workspace"
import { sql } from "@/lib/db"
import { WorkspaceError } from "@/lib/auth/workspace-context"

/** Directory saves go to the existing workspace CRM, not the retired outreach table. */
export async function saveDiscoveryContact(entityId: string, kind: "investor" | "firm") {
  ...
  const result = await sql`
    INSERT INTO crm_entries (org_id, user_id, source, import_key, firm_id, investor_id, display_name, display_email, stage)
    VALUES (${scope.orgId}, ${scope.userId}, ${source}, ${key}, ${kind === "firm" ? entityId : record.firm_id ?? null},
      ${kind === "investor" ? entityId : null}, ${name}, ${record.email ?? record.contact_email ?? null}, 'queued')
    ON CONFLICT (org_id, source, import_key) DO NOTHING RETURNING id
  `
  return { success: true as const, message: result.length ? "Added to your workspace CRM." : "Already in your workspace CRM. Existing notes and round links were kept." }
}
