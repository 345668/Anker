...
export async function getHomeData(userId: string) {
  ...
  if (fund) {
    ...
    const [totals, rows] = await Promise.all([
      sql`SELECT COUNT(*)::int total, COUNT(*) FILTER (WHERE stage NOT IN ('committed','passed'))::int active,
        ...
        FROM crm_entries WHERE org_id = ${active?.orgId ?? null} AND board_id = ${round.boardId}`,
      sql`SELECT id, display_name name, stage, check_size amount, updated_at FROM crm_entries
        WHERE org_id = ${active?.orgId ?? null} AND board_id = ${round.boardId} ORDER BY updated_at DESC, id LIMIT 5`,
    ])
    aggregate = totals[0] ?? {}; recent = rows
  }
  ...
  return {
    ...
    scope: fund ? `${active?.name} · ${fund.name}. Contacts are your workspace CRM records.` : round ? `${active?.name} · ${round.name} · ${round.currency}. Most recently created round. Contacts are your workspace CRM records.` : "Select a configured workspace to view its pipeline. Contacts are your workspace CRM records.",
  }
}
