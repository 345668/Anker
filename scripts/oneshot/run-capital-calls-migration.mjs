...
const sql = neon(url)
const sqlText = readFileSync("scripts/migrations/2026-06-21-capital-calls.sql", "utf8")
...
const cnt = await sql`
  SELECT
    (SELECT COUNT(*) FROM capital_calls)             AS calls,
    (SELECT COUNT(*) FROM capital_call_line_items)   AS line_items
`
console.log(cnt[0])
