 ...
 * not here.
 */
import { ingestProfile, draftByName, whoami, ingestConnections } from "~lib/anker-client";
...
chrome.runtime.onMessage.addListener((msg: { type: string; [k: string]: any }, _sender, sendResponse) => {
  (async () => {
    try {
      ...
      } else if (msg.type === "connections_ingest") {
        sendResponse(await ingestConnections(msg.connections));
      } else {
