...
export function AppSubnav() {
  ...
  useEffect(() => {
    try {
      setCollapsed(localStorage.getItem("anker:subnav") === "collapsed");
    } catch {
      /* ignore */
    }
    setReady(true);
  ...
  function toggle() {
    setCollapsed((c) => {
      const next = !c;
      try {
        localStorage.setItem("anker:subnav", next ? "collapsed" : "open");
      } catch {
        /* ignore */
      }
      return next;
  ...
  return (
    <aside
      className={`hidden md:flex sticky top-16 h-[calc(100dvh-4rem)] flex-col shrink-0 border-r border-border bg-card transition-[width] duration-200 ${collapsed ? "w-14" : "w-60"} ${ready ? "" : "invisible"}`}
    >
      <nav
      ...
      >
        {groups.map((g) => (
          <div key={g.heading} className="mb-4">
            {!collapsed && (
              <div className="text-xs font-mono uppercase tracking-[0.15em] text-muted-foreground px-3 mb-1.5">
                {g.heading}
              </div>
            )}
                ...
                return (
                  <li key={it.href + it.label}>
                    <Link
                      ...
                      className={`flex items-center gap-2.5 min-h-11 rounded px-3 py-2 text-sm transition-colors ${on ? "bg-accent text-accent-foreground font-medium" : "text-muted-foreground hover:text-foreground hover:bg-foreground/[0.04]"} ${collapsed ? "justify-center px-0" : ""}`}
                    >
                      {it.icon && (
                        <it.icon
                          className={`w-4 h-4 shrink-0 ${on ? "text-[var(--platform-link)]" : ""}`}
                        />
                      )}
                      {!collapsed && (
