...
function CallCard({ call, scope, refresh }: { call: Call; scope: CallScope; refresh: () => unknown }) {
  ...
  return <article className="border border-border rounded-md overflow-hidden">
    ...
    {open && <div id={`call-${call.id}`} className="p-5 border-t border-border space-y-5">
      ...
      {scope.persona !== "lp" && <div className="space-y-3"><label className="block text-sm">Linked contact · this workspace’s CRM (first 500)<select className={field} value={call.crm_entry_id ?? ""} disabled={!!busy || !scope.writable || !!call.outreach_message_id} onChange={e => action("link", { crmEntryId: e.target.value })}><option value="">Choose contact</option>{contacts?.contacts.map(c => <option key={c.id} value={c.id}>{c.display_name}</option>)}</select></label>{contactsError && <p role="alert">Contacts could not load. Reopen the call to retry.</p>}
