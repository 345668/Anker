...
export function DataTable<T>({
  ...
  function exportCsv() {
    ...
    const body = sorted
      .map((r) =>
        visible
          .map((c) => `"${String(val(c, r) ?? "").replace(/"/g, '""')}"`)
          .join(","),
      )
      .join("\n");
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <div className="relative w-full sm:w-auto">
          ...
          <input
            ...
            className="w-full sm:w-56 min-h-11 pl-8 pr-3 py-2 text-sm bg-background border border-foreground/15 rounded-md focus:border-foreground/40 outline-none placeholder:text-muted-foreground"
          />
        </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="inline-flex min-h-11 items-center gap-2 px-3 py-2 text-sm border border-input rounded hover:bg-accent"
              type="button"
            >
            ...
            {columns.map((c) => (
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={(checked) =>
                  setHidden((h) => {
                    const n = new Set(h);
                    checked ? n.delete(c.key) : n.add(c.key);
                    return n;
                  })
                }
                className="min-h-11"
        ...
        <button
          onClick={exportCsv}
          className="inline-flex min-h-11 items-center gap-2 px-3 py-2 text-sm border border-foreground/15 rounded-md hover:border-foreground/40"
          type="button"
        >
        ...
        <table className="w-full text-sm border-collapse">
              ...
              {visible.map((c) => {
                ...
                return (
                  <th
                    ...
                    className={`px-3 py-2.5 text-xs font-mono uppercase tracking-wider text-muted-foreground whitespace-nowrap select-none ${c.numeric ? "text-right" : "text-left"} ${c.sortable === false ? "" : "cursor-pointer hover:text-foreground"}`}
                  >
                    <button
                      type="button"
                      disabled={c.sortable === false}
                      onClick={() => toggleSort(c)}
                      className={`inline-flex min-h-8 items-center gap-1 disabled:cursor-default ${c.numeric ? "flex-row-reverse" : ""}`}
                    >
                      {c.header}
                      {c.sortable === false ? null : sorted_ ? (
                        sortDir === "asc" ? (
          ...
          <tbody>
            {sorted.length === 0 ? (
              <tr>
                <td
                  colSpan={visible.length + (renderExpanded ? 1 : 0)}
                  className="px-3"
                >
                  <EmptyState
                    compact
