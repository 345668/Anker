    ...
    <div>
      ...
      <div className="mt-8 overflow-x-auto border border-foreground/10 rounded-lg">
        <table className="w-full text-sm">
          ...
          <tbody>
            {vals.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-muted-foreground">No valuations yet. Request your first 409A above.</td></tr>
            ) : vals.map((v) => (
              <tr key={v.id} className="border-b border-foreground/[0.06] last:border-0">
                <td className="px-4 py-2.5 text-muted-foreground">{fmtD(v.created_at)}</td>
