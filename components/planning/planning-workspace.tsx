...
function Editor({ tool, scope, reload }: { tool: "runway" | "cap-table"; scope: { record: RecordState; orgId: string; name: string; canWrite: boolean }; reload: () => Promise<RecordState> }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-8">
    <header><p className="text-xs uppercase tracking-widest text-muted-foreground">{scope.name} / Personal planning</p><h1 className="mt-3 font-serif text-4xl">{tool === "runway" ? "Know when you need to raise" : "Understand your ownership"}</h1>
      <p className="mt-3 text-muted-foreground">{tool === "runway" ? "Model monthly cash flow and a planned financing over 36 months." : "Model a priced equity round and its target option pool. SAFE and convertible-note conversion are outside this model."} These scenarios are private to you within this workspace. All amounts are USD.</p></header>
