...
export default async function LpLayout({
  ...
  if (memberships.length === 0) {
    ...
    return (
      <main className="platform-workspace min-h-screen bg-background text-foreground flex items-center justify-center p-8">
        <div className="max-w-md text-center space-y-3">
          <h1 className="font-display text-2xl tracking-tight">LP portal</h1>
          <p className="text-sm text-muted-foreground">
            The email <span className="font-mono text-foreground">{email}</span>{" "}
            isn't attached to any LP on a fund we manage.
          </p>
          <p className="text-xs text-muted-foreground">
            If you expect access, contact the General Partner so they can attach
            your contact to your LP profile.
          </p>
          <div className="pt-4">
            <LogoutButton />
          </div>
  ...
  return (
    <div className="platform-workspace min-h-screen bg-background text-foreground">
      <a className="platform-skip" href="#workspace-content">
        Skip to workspace
      </a>
      <header className="border-b border-foreground/10 bg-background">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 py-4 flex flex-wrap items-center justify-between gap-3">
          <Link
            href="/lp"
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs font-mono uppercase tracking-wider text-muted-foreground truncate">
                {oversight
                  ? `Owner oversight · ${memberships.length} LP${memberships.length === 1 ? "" : "s"}`
                  : memberships.length === 1
                    ? memberships[0].fund_name
                    : `${memberships.length} funds`}
              </div>
            </div>
          </Link>
          <div className="flex items-center gap-2 shrink-0">
            {staff && (
              <Link
                href="/dashboard"
                className="inline-flex items-center gap-1.5 min-h-11 px-3 text-xs rounded-md border border-foreground/15 text-muted-foreground hover:text-foreground hover:bg-foreground/[0.04] transition-colors"
              >
