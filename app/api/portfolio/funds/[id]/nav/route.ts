/**
 * GET /api/portfolio/funds/[id]/nav
 *   → { nav, totalCalled, totalDistributed, dpi, rvpi, tvpi, grossMoic, netIrr }
 *
 * Fund NAV + performance computed entirely from the record (positions,
 * valuation snapshots, paid call/distribution lines). Admin-gated.
 */
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const perf = await getFundPerformance(fundId)
    if (!perf) {
