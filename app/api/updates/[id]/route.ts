...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const scope = await requireUpdateWorkspace(false)
  const user = { id: scope.userId }
  const [update] = (await sql`SELECT * FROM investor_updates WHERE id = ${id} AND org_id = ${scope.orgId} LIMIT 1`) as any[]
  ...
  `
  const recommended = !update.delivery_snapshot && update.status === "draft" ? await recommendRecipients(scope.orgId) : []
  return NextResponse.json({ update, recipients, recommended, canWrite: scope.canWrite, canSend: scope.canWrite && scope.canSendOutreach })
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
  const { id } = await params
  const scope = await requireUpdateWorkspace(true)
  const user = { id: scope.userId }
  ...
  const rows = (await sql`
    ...
    WHERE id = ${id} AND org_id = ${scope.orgId} AND status = 'draft' AND delivery_snapshot IS NULL AND revision = ${b.revision}
    RETURNING id
  `) as any[]
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const scope = await requireUpdateWorkspace(true)
  const user = { id: scope.userId }
  const rows = (await sql`DELETE FROM investor_updates WHERE id = ${id} AND org_id = ${scope.orgId} AND status <> 'sending' RETURNING id`) as any[]
